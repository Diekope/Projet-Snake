...
int partie(){
    ...
    // ===La_création===
    SDL_Window* window = SDL_CreateWindow(
        "Le Snake de la Diversité - Cartes", // ===Le_titre_de_la_fenêtre===
        SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
        lg, lag,
        SDL_WINDOW_SHOWN
    );
    ...
    Button play = {{100, 100, 200, 50}, "Nouvelle Partie"}; // Position et taille originales
    play.rect.x = 40;
    play.rect.y = (lag - play.rect.h) / 2;

    Button autre = {{100, 100, 200, 50}, "Charger une partie"};
    autre.rect.x = play.rect.x+300;
    autre.rect.y = play.rect.y;
